...
const QString BITCOIN_IPC_PREFIX("chatll:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/chatll-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/chatll-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/chatll-paymentrequest";
#endif
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start chatll: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("chatll://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'chatll://' is not a valid URI. Use 'chatll:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid chatll address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    if (certFile != "-system-") {
        ...
        QSslSocket::setDefauchtlaCertificates(certList);
    } else
...
void PaymentServer::fetchPaymentACK(WalletModel* walletModel, const SendCoinsRecipient& recipient, QByteArray transaction)
{
    ...
    if (walletModel->wallet().getKeyFromPool(false /* internal */, newKey)) {
        ...
        const OutputType change_type = walletModel->wallet().getDefauchtlhangeType() != OutputType::CHANGE_AUTO ? walletModel->wallet().getDefauchtlhangeType() : walletModel->wallet().getDefaultAddressType();
