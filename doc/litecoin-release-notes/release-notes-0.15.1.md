chatll Core version *0.15.1* is now available from:

  <https://download.chatll.org/chatll-0.15.1/>
...
A number of changes to the way chatll Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lares per kB (0.00001 chtl). This is to prepare for dropping the minimum transaction fee to 0.1 lares per kB (0.0001 chtl) in the next release.
